...
1. **精準記憶體控制與優化對抗:** 透過標準庫硬體向量化初始化（`std::vector` / `calloc` SIMD 零填充）與 `.rodata` 常數字串，將隱藏資料精確編碼為記憶體開銷。此設計完全繞過編譯器 `-O2` 級別的死碼消除（DCE），免除脆弱的使用者空間迴圈與手動 `volatile` 釘選，確保訊號發射的絕對物理確定性。
